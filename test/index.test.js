...
const inputValues = {
  ...
  URLWithTrailingDot: 'You can read https://github.com/huckbit/extract-urls. Or try https://www.npmjs.com/package/extract-urls. For more info try GitHub.`',
  URLTopLevelDomain: 'Urls with long top level domain https://www.corsenetinfos.corsica, https://www.example.somerandomtopleveldomain/',
};
...
const expectedValues = {
  ...
  URLSWithTopLevelDomain: ['https://www.corsenetinfos.corsica', 'https://www.example.somerandomtopleveldomain/'],
};
...
test('Match URL with trailing dot removed from URL', () => {
  expect(extractUrls(inputValues.URLWithTrailingDot)).toEqual(expect.arrayContaining(expectedValues.URLsList));
});

test('Match long top level domain (issue)', () => {
  expect(extractUrls(inputValues.URLTopLevelDomain)).toEqual(expect.arrayContaining(expectedValues.URLSWithTopLevelDomain));
});
